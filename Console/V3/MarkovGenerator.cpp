...
void MarkovGenerator::train(const std::vector<std::string>& pWordPool)
{
...
std::string MarkovGenerator::generate()
{
	...
	while (true)
	{
		...
		char nextChar = options[getRandomInt(options.size())];
		if (nextChar == '_')
		{
			break;
		}
	...
	if (result.empty())
	{
		return "No valid name generated";
	}
	else
	{
		return result;
